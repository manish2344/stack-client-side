...
export const profileAction = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`https://stack-overflow-063y.onrender.com/api/user/user/${id}`);
...
export const editAction = (id , credentials) => async (dispatch) =>  {
  try {
    ...
    const {data}  = await axios.put(`https://stack-overflow-063y.onrender.com/api/user/user/${id}` , credentials , config)
