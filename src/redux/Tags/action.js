...
export const tagAction = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`https://stack-overflow-063y.onrender.com/api/question/tags`);
...
export const searchTagsAction = (value) => async (dispatch) => {
  try {
    dispatch({ type: TAG_REQUEST });
    const { data } = await axios.get(`https://stack-overflow-063y.onrender.com/api/question/searchTags?search=${value}`);
